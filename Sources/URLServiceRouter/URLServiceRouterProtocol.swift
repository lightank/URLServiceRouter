...
public typealias URLServiceNodeParsersBuilder = () -> [URLServiceNodeParserProtocol]
...
public protocol URLServiceRouterProtocol {
    /// 返回将要在根节点中注册的解析器数组
    var rootNodeParsersBuilder: URLServiceNodeParsersBuilder? { get set }

    // MARK: 代理

    /// 代理，提供：当前vc信息、拦截请求、动态处理请求结果、记录日志等功能
    var delegate: URLServiceRouterDelegateProtocol? { get set }
    ...
    func registerNode(from url: String, parsersBuilder: URLServiceNodeParsersBuilder?)
